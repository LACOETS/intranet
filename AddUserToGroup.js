...
//Call Approvers List - Master List to get group name
...
//alert('In AddUserUsingREST:=' + groupName + " " + editorNames);
groupID = "1671";
...
SP.SOD.executeFunc("sp.js", "SP.ClientContext", function() {
    SP.SOD.executeFunc("sp.runtime.js", "SP.ClientContext", function() {      	  
	  var queryUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/sitegroups/getbyname('Technology Services Approvers')";   //?$select=id";
	  //alert(queryUrl);      
      $.ajax({
        url: queryUrl,
        method: "GET",
...
function onQuerySuccess2(data) {		
		var results = data.d.results;
		alert('In onQuerySuccess2:=' + results);
		$.each(results, function(index, dataRec) {
		var userEntry = {}; 
